...
if [[ "${REPLY}" =~ ^[Yy]$ ]]; then
    ...
    BACKUP_FILE="${CONFIG_FILE}.gemini-backup.$(date +%s)"
    ...
    if bash -n "${CONFIG_FILE}" 2>/dev/null; then
        echo "✅ Shell configuration validated successfully."
        echo "Please run 'source ${CONFIG_FILE}' or open a new terminal to use the 'gemini' command."
    else
        echo "⚠️  WARNING: Shell config may have syntax errors."
        echo "Restoring backup..."
        mv "${BACKUP_FILE}" "${CONFIG_FILE}"
        echo "Backup restored. Please check the alias and add it manually."
        exit 1
    fi
else
