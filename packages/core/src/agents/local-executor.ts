...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private readonly agentsUsed = new Set<string>();
  ...
  static async create<TOutput extends z.ZodTypeAny>(
    ...
    // Register the delegation tool for this agent.
    agentToolRegistry.registerTool(
      new DelegateTool(context.config, subagentMessageBus),
    );
  ...
  private async runInternal(
    ...
    try {
      ...
      // Log the turn to project context if applicable
      const projectContextService = new ProjectContextService(this.context.config);
      if (await projectContextService.contextExists()) {
        const summary = finalResult || 'Agent execution finished.';
        await projectContextService.logTurn(query, summary, Array.from(this.agentsUsed));
      }
  ...
  private async processFunctionCalls(
    ...
    for (const [index, functionCall] of functionCalls.entries()) {
      ...
      if (toolName === DELEGATE_TOOL_NAME && typeof args['agent'] === 'string') {
        this.agentsUsed.add(args['agent']);
      }
    }
  ...
  private async buildSystemPrompt(inputs: AgentInputs): Promise<string> {
    const overridePrompt = this.context.config.getSystemPrompt();
    if (overridePrompt) {
      return templateString(overridePrompt, inputs);
    }
...
* If a tool call is rejected by the user, acknowledge the rejection, rethink your strategy, and try a different approach. Do not repeatedly attempt the same rejected operation.`;
    ...
    // Inject delegation guidance if the delegate tool is available.
    if (this.toolRegistry.getTool(DELEGATE_TOOL_NAME) !== undefined) {
      finalPrompt += `

# Multi-Agent Orchestration
You have access to specialized sub-agents via the \`delegate\` tool. Use them to multiply your output volume and speed. You are the Primary Agent responsible for planning, architecture, and final assembly.

## Decision Engine
1. **Needs careful reasoning/judgment?** -> You (Primary) handle it.
2. **Expected output > 600 tokens?** -> DELEGATE to a sub-agent.
3. **Can task be broken into independent chunks?** -> DELEGATE tasks in PARALLEL.
4. **Repetitive batch work?** -> DELEGATE.

## Sub-Agent Personas
* **gmi2**: Your "Workhorse". Use for high-volume code output, boilerplate, tests, and heavy transformations.
* **gmi3**: Your "Researcher/Writer". Use for deep research, documentation drafting, and complex content analysis.

## Delegation Strategy
* Use the \`delegate\` tool to send a list of \`tasks\`.
* For independent parallel work, include multiple tasks in a single \`delegate\` call.
* Sub-agents are strict workers: they will execute completely and return clean output without meta-commentary.`;
    }
