...
export class Storage {
  ...
  private static getSystemConfigDir(): string {
    ...
    if (os.platform() === 'darwin') {
      systemDir = '/Library/Application Support/GeminiCli';
    } else if (os.platform() === 'win32') {
      systemDir = 'C:\\ProgramData\\gemini-cli';
    } else {
      systemDir = '/etc/gemini-cli';
    }
    
    // Test if we can write to system directory
    try {
      const testPath = path.join(systemDir, '.write_test');
      ...
      return path.join(homedir(), '.gemini', 'system-config');
    }
  }
  ...
  getContextDir(): string {
    return path.join(this.targetDir, '.gemini-context');
  }

  getProjectMdPath(): string {
    return path.join(this.getContextDir(), 'PROJECT.md');
  }

  getSessionLogPath(): string {
    return path.join(this.getContextDir(), 'SESSION_LOG.md');
  }
