 ...
 * If GEMINI_CLI_HOME or GEMINI_CONFIG_DIR environment variable is set, it returns its value.
 * Otherwise, it returns the user's home directory.
 */
export function homedir(): string {
  const envHome = process.env['GEMINI_CLI_HOME'] || process.env['GEMINI_CONFIG_DIR'];
  if (envHome) {
    return envHome;
  }
  return os.homedir();
