...
export async function initializeApp(
  ...
  const projectContextService = new ProjectContextService(config);
  if (await projectContextService.contextExists()) {
    await projectContextService.logSessionStart();
  } else if (config.isInteractive()) {
    coreEvents.emitFeedback(
      'info',
      'No project context found. Use "gmi context init" to initialize one for this directory.',
    );
  }
