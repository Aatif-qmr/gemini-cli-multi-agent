...
The agent did not use the todo list because this task could be completed by a tight loop of execute test->edit->execute test.
  ...
  delegate: {
    name: 'delegate',
    description: 'Delegate complex or high-volume tasks to specialized sub-agents (gmi2 or gmi3) in parallel.',
    parametersJsonSchema: {
      type: 'object',
      properties: {
        tasks: {
          type: 'array',
          description: 'A list of tasks to execute in parallel.',
          items: {
            type: 'object',
            properties: {
              agent: {
              ...
              prompt: {
                type: 'string',
                description: 'The complete prompt/task for the sub-agent.',
              },
            },
