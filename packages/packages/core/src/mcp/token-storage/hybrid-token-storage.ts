...
export class HybridTokenStorage extends BaseTokenStorage {
  ...
  constructor(serviceName: string) {
    // If GEMINI_CLI_HOME is set, use it to isolate the keychain service name.
    // This allows multiple agents to have isolated accounts even on the same OS user.
    let isolatedServiceName = serviceName;
    const cliHome = process.env['GEMINI_CLI_HOME'];
    if (cliHome) {
      const agentDirName = path.basename(cliHome);
      if (agentDirName.startsWith('.')) {
        isolatedServiceName = `${serviceName}.${agentDirName.substring(1)}`;
      }
    }
    super(isolatedServiceName);
  }
